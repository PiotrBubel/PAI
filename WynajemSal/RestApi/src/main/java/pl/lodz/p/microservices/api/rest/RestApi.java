...
import pl.lodz.p.microservices.api.rest.method.RoomsManagementMethods;
...
public class RestApi extends AbstractVerticle {
    private static final String ROOMS_MANAGEMENT_SERVICE_ADDRESS = "pl.lodz.p.microservices.management.rooms.RoomsManagement";
    ...
    private static final String ROOMS_ENDPOINT = "/api/rooms";
    ...
    public void start() {
        ...
        // Rooms endpoint
        router.get(ROOMS_ENDPOINT).handler(context -> requestHandler(context,
                RoomsManagementMethods.GET_ROOMS_LIST,
                ROOMS_MANAGEMENT_SERVICE_ADDRESS,
                false));

        router.get(ROOMS_ENDPOINT + "/:name").handler(context -> requestHandler(context,
                RoomsManagementMethods.GET_ROOM_DETAILS,
                ROOMS_MANAGEMENT_SERVICE_ADDRESS,
                "name",
                false));

        router.delete(ROOMS_ENDPOINT + "/:name").handler(context -> requestHandler(context,
                RoomsManagementMethods.DELETE_ROOM,
                ROOMS_MANAGEMENT_SERVICE_ADDRESS,
                "name",
                true));

        router.post(ROOMS_ENDPOINT).handler(context -> requestHandler(context,
                RoomsManagementMethods.SAVE_NEW_ROOM,
                ROOMS_MANAGEMENT_SERVICE_ADDRESS,
                parseBodyToJson(context.getBodyAsString()),
                true));

        router.put(ROOMS_ENDPOINT + "/:name").handler(context -> requestHandler(context,
                RoomsManagementMethods.EDIT_ROOM,
                ROOMS_MANAGEMENT_SERVICE_ADDRESS,
                "name",
                parseBodyToJson(context.getBodyAsString()),
                true));
